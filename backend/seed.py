...
async def seed_data():
    ...
    for j, (title, category, number, unit) in enumerate(doc_info):
        await db.documents.insert_one({'id': f'doc-{j}', 'title': title, 'category': category, 'number': number, 'unit': unit, 'description': 'Materi contoh untuk evaluasi pemahaman regulasi. Dokumen ini bukan regulasi resmi bank. Setiap pegawai wajib memahami prinsip kehati-hatian, pelindungan data nasabah, dan tata kelola yang baik.', 'status': 'published' if j < 6 else 'pending', 'version': '1.0' if j < 6 else '2.0', 'filename': None, 'created_by': 'demo-admin', 'created_at': (datetime.now(timezone.utc)-timedelta(days=j*5)).isoformat(), 'updated_at': now(), 'is_deleted': False, 'sample': True})
    ...
    for j, title in enumerate(titles):
        ...
        questions = [
            {'id': 'q1', 'text': 'Apa tujuan utama penerapan regulasi kepatuhan di bank?', 'type': 'multiple', 'options': ['Mempercepat transaksi tanpa pemeriksaan', 'Memastikan kegiatan bank sesuai peraturan yang berlaku', 'Mengurangi jumlah nasabah', 'Menghilangkan proses dokumentasi'], 'correct': 1},
            ...
            {'id': 'q4', 'text': 'Kapan pegawai perlu mempelajari pembaruan regulasi?', 'type': 'multiple', 'options': ['Segera setelah regulasi dipublikasikan', 'Hanya saat audit', 'Setelah terjadi pelanggaran', 'Tidak perlu jika sudah berpengalaman'], 'correct': 0}]
        quiz = {'id': f'quiz-{j}', 'title': title, 'description': 'Evaluasi pemahaman regulasi dan penerapannya dalam pekerjaan sehari-hari.', 'document_id': f'doc-{j}', 'passing_grade': 75, 'duration': 30, 'units': [], 'positions': [], 'start_date': start.date().isoformat(), 'end_date': end.date().isoformat(), 'status': 'published', 'questions': questions, 'created_by': 'demo-admin', 'created_at': start.isoformat(), 'updated_at': now(), 'is_deleted': False}
    ...
    await db.notifications.insert_one({'id': uid(), 'title': '2 regulasi menunggu persetujuan', 'detail': 'Pembaruan regulasi siap ditinjau oleh Supervisor SISDUR.', 'roles': ['administrator','supervisor'], 'user_ids': [], 'read_by': [], 'link': '/regulasi?status=pending', 'created_at': now()})
