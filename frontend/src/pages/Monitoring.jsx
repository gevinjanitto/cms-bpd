...
export default function Monitoring({ user }) {
  ...
  return <>
    <PageHead eyebrow="ANALITIK & EVALUASI" title="Monitoring & Laporan" subtitle="Pantau partisipasi, hasil evaluasi, dan pemahaman regulasi di seluruh unit kerja."><ExportMenu id="monitoring-export" params={{ unit, period, view: tab, q, quiz_id: resultTab ? quizId : '' }} />{user.role !== 'director' && <Btn data-testid="monitoring-remind" onClick={() => setRemindConfirm(true)}><Bell size={15} />Kirim Pengingat</Btn>}</PageHead>
