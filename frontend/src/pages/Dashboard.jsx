...
export default function Dashboard({ user }) {
  ...
  return <>
    ...
    <div className="stats-grid">
      <Stat id="documents" label="Regulasi Aktif" value={m.documents} icon={Files} detail={<><span className="mini-dot green" />{m.internal} Internal<span className="detail-divider" /><span className="mini-dot blue" />{m.external} Eksternal</>} />
    ...
    <div className="bottom-grid"><section className="unit-section"><SectionHead title={employee ? 'Regulasi Terbaru' : 'Performa Unit Kerja'} subtitle={employee ? 'Materi kepatuhan yang telah dipublikasikan' : 'Partisipasi dan hasil evaluasi per unit kerja'}><ArrowLink data-testid="dashboard-view-units" onClick={() => navigate(employee ? '/regulasi' : '/monitoring')}>Lihat semua</ArrowLink></SectionHead>
      ...
      {!employee && <><button className="attention-item" data-testid="attention-documents" onClick={() => navigate('/regulasi?status=pending')}><span className="attention-icon amber"><Files size={18} /></span><div><strong>{data.tasks.documents} regulasi menunggu review</strong><small>Periksa dan tinjau dokumen baru</small></div><ChevronRight size={15} /></button><button className="attention-item" data-testid="attention-essays" onClick={() => navigate('/monitoring?tab=essay')}><span className="attention-icon blue"><ClipboardCheck size={18} /></span><div><strong>{data.tasks.essays} esai belum dinilai</strong><small>Lengkapi hasil evaluasi karyawan</small></div><ChevronRight size={15} /></button></>}
